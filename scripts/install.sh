...
if [ $exitstatus = 0 ]; then
    ...
    if [ $OPTION = "1" ]; then
        curl -sSL https://raw.githubusercontent.com/scidsg/hushline/main/scripts/install-tor-only.sh | bash
    elif [ $OPTION = "2" ]; then
        curl -sSL https://raw.githubusercontent.com/scidsg/hushline/main/scripts/install-public-plus-tor.sh | bash
    fi
else
