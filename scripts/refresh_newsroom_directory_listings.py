...
def _default_output_path() -> Path:
    return _project_root() / "hushline" / "data" / "newsroom_directory_listings.json"
...
def _load_existing_rows(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(parsed, list):
        raise NewsroomDirectoryRefreshError(f"Existing output root must be a JSON array: {path}")
...
def main() -> int:
    ...
    fetched_rows = fetch_newsroom_directory_rows(timeout_seconds=args.timeout_seconds)
    refreshed_rows = refresh_newsroom_directory_rows(fetched_rows)
    ...
    if args.check and output_content != existing_content:
        raise NewsroomDirectoryRefreshError(f"Output file is out of date: {args.output}")
...
if __name__ == "__main__":
    ...
    except NewsroomDirectoryRefreshError as exc:
