...
MODEL="${AIDER_MODEL:-ollama:qwen2.5-coder:7b-instruct}"  # colon format for aider
...
# Candidate files from issue text (scss, py, js, html, etc.)
TARGET_FILES=()
...
for attempt in 1 2; do
  ...
  if make lint >/dev/null 2> "$LINT_LOG"; then
    ...
    FEEDBACK="$(<"$LINT_LOG")"
    echo "Lint failed, retry $attempt..."
