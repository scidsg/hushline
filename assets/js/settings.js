...
async function provisionChatKey(form) {
  ...
  const action = form.dataset.chatKeyAction === "rotate" ? "rotate" : "create";

  if (action === "rotate" && form.dataset.confirmMessage) {
    const confirmed = confirm(form.dataset.confirmMessage);
    if (!confirmed) {
      return;
    }
  }
