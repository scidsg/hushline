...
test("narrative outline and high-risk next actions use a review gate", async ({
  ...
  await page
    .getByRole("radio", { name: "Pause while keeping this page open" })
    .check();
  await page.locator("#case-next-action-review").click();
  await expect(actionReview).toContainText(
    "Pause while keeping this page open review",
  );
  await expect(actionReview).toContainText("Keep this page open to pause");

  await page.getByRole("radio", { name: "Decide not to proceed" }).check();
  await page.locator("#case-next-action-review").click();
  await expect(actionReview).toContainText("Decide not to proceed review");
  await expect(actionReview).toContainText("stop without sharing anything");
  await expect(
    actionReview.getByRole("link", { name: "Discard workspace and leave" }),
