...
def test_profile_location_returns_none_for_inconsistent_empty_us_geography() -> None:
    class FalsyUnitedStates(str):
        def __new__(cls) -> "FalsyUnitedStates":
            return super().__new__(cls, "United States")
    ...
    with patch(
        ...
        return_value=SimpleNamespace(
            location="United States",
            country=FalsyUnitedStates(),
            city=None,
            subdivision=None,
            subdivision_code=None,
        ),
    ):
        assert user.profile_location is None
