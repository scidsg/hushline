...
def encrypt_field(data: str | None) -> str | None:
    if data is None:
        return None
...
def is_valid_pgp_key(key: str) -> bool:
    ...
    try:
        ...
        return bool(imported_key.count > 0)
    except Exception as e:
        current_app.logger.error(f"Error importing PGP key: {e}")
        return False


def encrypt_message(message: str | bytes, recipient_email: str) -> str | None:
    ...
    try:
        ...
        encrypted_data = gpg.encrypt(message_bytes, recipients=recipient_email, always_trust=True)
...
def get_email_from_pgp_key(pgp_key: str) -> str | None:
    gpg = gnupg.GPG(gpg_home)
    try:
        # Import the PGP key
            ...
            for key in all_keys:
                if key["keyid"] == key_id:
                    ...
                    if email_start > 0 and email_end > email_start:
                        return uids[email_start:email_end]  # type: ignore
    except Exception as e:
