from __future__ import annotations

import json
import unicodedata
from dataclasses import dataclass
from functools import lru_cache
from pathlib import Path
from typing import Any

from unidecode import unidecode


@dataclass(frozen=True)
class SecureDropDirectoryListing:
    id: str
    slug: str
    name: str
    website: str
    description: str
    directory_url: str
    landing_page_url: str
    onion_address: str
    onion_name: str
    countries: tuple[str, ...]
    languages: tuple[str, ...]
    topics: tuple[str, ...]
    source_label: str
    source_url: str
    listing_type: str = "securedrop_instance"
    directory_section: str = "securedrop_directory"
    message_capable: bool = False
    is_automated: bool = True

    @property
    def location(self) -> str:
        if not self.countries:
            return "Unknown"
        return ", ".join(self.countries)


def _sort_key(value: str) -> str:
    normalized = unicodedata.normalize("NFKC", value.strip())
    return unidecode(normalized).casefold()


def _seed_path() -> Path:
    return Path(__file__).resolve().parent.parent / "data" / "securedrop_directory_instances.json"


@lru_cache(maxsize=1)
def get_securedrop_directory_listings() -> tuple[SecureDropDirectoryListing, ...]:
    path = _seed_path()
    if not path.exists():
        return ()

    rows = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(rows, list):
        return ()

    listings = tuple(_build_listing(row) for row in rows if isinstance(row, dict))
    return tuple(sorted(listings, key=lambda listing: (_sort_key(listing.name), listing.id)))


def get_securedrop_directory_listing(slug: str) -> SecureDropDirectoryListing | None:
    slug_casefold = slug.casefold()
    return next(
        (
            listing
            for listing in get_securedrop_directory_listings()
            if listing.slug.casefold() == slug_casefold
        ),
        None,
    )


def _build_listing(row: dict[str, Any]) -> SecureDropDirectoryListing:
    return SecureDropDirectoryListing(
        id=row["id"],
        slug=row["slug"],
        name=row["name"],
        website=row["website"],
        description=row["description"],
        directory_url=row["directory_url"],
        landing_page_url=row["landing_page_url"],
        onion_address=row["onion_address"],
        onion_name=row.get("onion_name", ""),
        countries=tuple(row.get("countries", [])),
        languages=tuple(row.get("languages", [])),
        topics=tuple(row.get("topics", [])),
        source_label=row["source_label"],
        source_url=row["source_url"],
    )
