...
- `.github/workflows/public-directory-weekly-report.yml` fetches `https://hushline.app/directory/users.json`, filters to opted-in public Hush Line users, compares against the last `hushline-stats` snapshot, then updates `scidsg/hushline-stats` with both latest aliases and dated historical JSON/Markdown reports.
