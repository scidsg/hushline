...
- `.github/workflows/public-directory-weekly-report.yml` can run weekly or on demand, fetches `https://tips.hushline.app/directory/users.json`, filters to opted-in public Hush Line users, compares against both the last sync and the most recent snapshot from at least 7 days earlier, then updates `scidsg/hushline-stats` with latest aliases and timestamped historical JSON/Markdown reports.
