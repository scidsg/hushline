...
- `.github/workflows/docs-screenshots.yml` is the release and manual entrypoint for screenshots. On published releases it waits for the released GHCR image, captures screenshots for that tag, and then calls `.github/workflows/publish-docs-screenshots.yml` to sync them into both `scidsg/hushline-website` and `scidsg/hushline-screenshots` via dedicated automation branches and PRs. The workflow attempts to merge those PRs immediately so the website repo keeps the latest screenshots at `src/assets/img/screenshots/` and archives `src/assets/img/screenshots/releases/latest/` plus `src/assets/img/screenshots/releases/vX.Y.Z/`, while the screenshots repo keeps the standalone release archive and badge.
