...
- `HUSHLINE_DAILY_CODEX_STATUS_IDLE_CHECK_STATE_FILE` (default `${HUSHLINE_DAILY_RUNNER_LOCK_DIR}/codex-status-last-check`; stores the last idle `/status` attempt timestamp in the runner lock directory)
